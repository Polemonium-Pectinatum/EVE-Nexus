...
struct SettingView: View {
    ...
    private func deleteIconsAndRestart() {
        Task {
            ...
            do {
                ...
                // 3. 获取 Bundle 中的 icons.zip 路径并计算 SHA256
                ...
                // 4. 计算 Bundle 图标包的 SHA256
                let bundleIconHash = calculateIconsSHA256(filePath: bundleIconPath)
                Logger.info("Bundle icons.zip SHA256: \(bundleIconHash ?? "nil")")
                ...
                // 6. 保存 Bundle 图标包的 SHA256 到 UserDefaults
                if let hash = bundleIconHash {
                    UserDefaults.standard.set(hash, forKey: "IconsZipHash")
                    Logger.info("Updated IconsZipHash to Bundle hash: \(hash)")
                }
    ...
    // 计算图标包 SHA256
    private func calculateIconsSHA256(filePath: String) -> String? {
                    ...
                    Section {
                        ...
                        HStack {
                            Text(NSLocalizedString("SDE_Current_Version", comment: "当前版本"))
                                .font(.system(size: 16))
                            Spacer()
                            Text(updateChecker.currentIconsHash.isEmpty ?
                                NSLocalizedString("SDE_Not_Available", comment: "不可用") :
                                updateChecker.currentIconsHash)
                                .font(.system(size: 16, weight: .medium))
                                .foregroundColor(
                                    !updateChecker.currentIconsHash.isEmpty &&
                        ...
                        HStack {
                            Text(NSLocalizedString("SDE_Latest_Version", comment: "最新版本"))
                                .font(.system(size: 16))
                            Spacer()
                            Text(updateChecker.latestIconsHash.isEmpty ?
                                NSLocalizedString("SDE_Not_Available", comment: "不可用") :
                                updateChecker.latestIconsHash)
                                .font(.system(size: 16, weight: .medium))
                                .foregroundColor(
                                    !updateChecker.currentIconsHash.isEmpty &&
                                        !updateChecker.latestIconsHash.isEmpty &&
